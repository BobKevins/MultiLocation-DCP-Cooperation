function result = solve_stage2_users_scalability(cfg, dc, stage1_result)
%SOLVE_STAGE2_USERS_SCALABILITY 求解 R2C4 单个 DCP 的扩展 Stage 2 模型。
%
...
%% ================================== 00 可配置代表性用户组 ==================================
[workload_inter, workload_batch, group_metadata] = build_stage2_user_groups( ...
    cfg, dc, stage1_result);

number_user_group = size(workload_inter, 1);                                  % 代表性用户组数量
...
zero_workload_tolerance = cfg.stage2.zero_workload_tolerance;                  % 数值零负载阈值，request/s
zero_inter_mask = workload_inter <= zero_workload_tolerance;                   % 交互式零负载用户--时段
zero_batch_mask = workload_batch <= zero_workload_tolerance;                   % 批处理零负载用户--时段
candidate_active = false(number_user_group, number_combination, number_period); % 预处理后有效延时组合

original_inter_mask = abs(inter_delay_candidate - ...
    cfg.time_tolerance_inter_original) <= 1e-12;
original_batch_mask = abs(batch_delay_candidate - ...
    cfg.time_tolerance_batch_original) <= 1e-12;

% 零负载分量的延时不产生物理或经济影响，固定为原始档位以消除并列整数解。
for user_group = 1:number_user_group
    for t = 1:number_period
        if zero_inter_mask(user_group, t) && zero_batch_mask(user_group, t)
            active_mask = original_inter_mask & original_batch_mask;
        elseif zero_inter_mask(user_group, t)
            active_mask = original_inter_mask;
        elseif zero_batch_mask(user_group, t)
            active_mask = original_batch_mask;
        else
            active_mask = true(number_combination, 1);
        end
        candidate_active(user_group, :, t) = reshape(active_mask, 1, []);
    end
end
...
for user_group = 1:number_user_group
    ...
    for t = 1:number_period
        active_mask = candidate_active(user_group, :, t);
        inactive_mask = ~active_mask;

        constraints = [constraints, ...
            sum(select_combination(user_group, active_mask, t)) == 1];          %#ok<AGROW>
        if any(inactive_mask)
            constraints = [constraints, ...
                select_combination(user_group, inactive_mask, t) == 0];        %#ok<AGROW>
        end
        constraints = [constraints, ...
        ...
        for combination = 1:number_combination
            ...
            % 无效候选组合不参与成本比较，选择变量和乘积辅助量直接置零。
            if ~candidate_active(user_group, combination, t)
                constraints = [constraints, ...
                    omega_i == 0, omega_b == 0, ...
                    net_cost_candidate(user_group, combination, t) == 0];      %#ok<AGROW>
                continue;
            end
...
for user_group = 1:number_user_group
    for t = 1:number_period
        active_index = find(candidate_active(user_group, :, t));
        for selected_index = active_index
            for alternative_index = active_index
...
for user_group = 1:number_user_group
    for source_time = 1:number_period
        % 来源时段无批处理任务时，固定整条转移轨迹为零。
        if zero_batch_mask(user_group, source_time)
            constraints = [constraints, ...
                batch_window_active(user_group, source_time, :) == 0, ...
                delta_batch(user_group, source_time, :) == 0];                %#ok<AGROW>
            continue;
        end
...
for user_group = 1:number_user_group
    for t = 1:number_period
        if zero_inter_mask(user_group, t)
            constraints = [constraints, ...
                server_inter_group(user_group, t) == 0];                       %#ok<AGROW>
        else
...
diagnostics = optimize(constraints, objective_stage2, cfg.stage2.solver);
...
result.number_user_group = number_user_group;
result.user_group_origin = group_metadata.group_origin;
result.workload_inter_group = workload_inter;
result.workload_batch_group = workload_batch;
result.zero_workload_tolerance = zero_workload_tolerance;
result.zero_inter_user_period_count = nnz(zero_inter_mask);
result.zero_batch_user_period_count = nnz(zero_batch_mask);
result.active_candidate_count = nnz(candidate_active);
result.full_candidate_count = numel(candidate_active);
result.pairwise_optimality_constraint_count = calculate_pairwise_count( ...
    candidate_active);
result.workload_accounting_inter_error = group_metadata.inter_accounting_error;
result.workload_accounting_batch_error = group_metadata.batch_accounting_error;
result.selected_inter_delay = value(selected_inter_delay);
result.selected_batch_delay = value(selected_batch_delay);
result.delta_batch = value(delta_batch);
...
function pairwise_count = calculate_pairwise_count(candidate_active)
%CALCULATE_PAIRWISE_COUNT 统计预处理后的成对最优性约束数量。

number_active = squeeze(sum(candidate_active, 2));
pairwise_count = sum(number_active(:) .* (number_active(:) - 1));
end
