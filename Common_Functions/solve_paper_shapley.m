function result = solve_paper_shapley(cfg, grand_stage2_results)
...
singleton_stage2 = cell(number_player, 1);                                  % 复用单成员联盟的独立运行结果
...
for coalition_mask = 1:grand_coalition_mask
    ...
    if isscalar(members)
        singleton_stage2{members(1)} = stage2_results{1};
    end
...
for coalition_mask = 1:grand_coalition_mask
    member_mask = logical(bitget(coalition_mask, 1:number_player));
    coalition_value(coalition_mask + 1) = ...
        sum(independent_cost(member_mask)) - coalition_cost(coalition_mask + 1);
end
...
result.singleton_stage2 = singleton_stage2;
